...
class News(BaseModel):
	...
	digest=models.CharField(max_length=256,null=False,verbose_name='新闻简要')
	content = models.TextField(null=False,verbose_name='新闻内容')
	clicks = models.IntegerField(default=0,verbose_name='文章点击数')
	status = models.SmallIntegerField(choices=NEWS_STATUS_CHOICES,default=1,verbose_name='文章审核状态')
	reason = models.CharField(max_length=256,null=True,verbose_name='审核不通过原因')# 未通过原因，status = 3 的时候使用
...
class User_Fans(models.Model):
	...
	用户关注表
	"""
	follow = models.ForeignKey(User,verbose_name='关注',on_delete=models.CASCADE,related_name='follows',primary_key=True)
...
class Comment_Relation(models.Model):
	...
	"""
	# 当有评论的回复就产生该集合
	commenter = models.ForeignKey(Comment,verbose_name='评论信息父id',related_name='commenter',primary_key=True)

	replier = models.ForeignKey(Comment,verbose_name='回复评论信息id',related_name='replier')
